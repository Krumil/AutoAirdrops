...
def randomize_transaction_parameters():
    ...
    destination_chain = random.choice(
        [chain for chain in chains if chain != starting_chain]
    )
...
def load_uniswap_token_list(file_path):
    with open(file_path, "r") as file:
        token_list = json.load(file)
    return token_list


def get_all_tokens(chain_name, account_address, token_list):
    tokens = []
    chain_id = get_chain_id(chain_name)
    for token in token_list["tokens"]:
        if token["chainId"] == chain_id:
            sleep(1)
            amount = get_token_balance(chain_name, account_address, token["symbol"])
            if amount > 0:
                tokens.append(
                    {
